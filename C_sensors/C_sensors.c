...
#define bit_delay_time 100//8.5 // bit delay for 115200 with overhead
...
int main(void) {
   ...
   while (1) {
	...
	//read sensors
	 ...
	 char_delay();
	 put_char(&serial_port, serial_pin_out, sense1_lo);
	 char_delay();
	 put_char(&serial_port, serial_pin_out, sense1_hi);
	 char_delay();
	 input(serial_direction, serial_pin_out);
      }
      }
